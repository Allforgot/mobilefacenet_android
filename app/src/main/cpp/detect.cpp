...
Detect::Detect(const std::vector<std::string> param_files,
...
void Detect::generateBbox(ncnn::Mat score, ncnn::Mat location, std::vector<Bbox> &boundingBox_,
                          float scale) {
	const int stride = 2;
...
void Detect::nms(std::vector<Bbox> &boundingBox_, const float overlap_threshold,
	...
	const unsigned int num_boxes = boundingBox_.size();
	vPick.resize(num_boxes);
	for (int i = 0; i < num_boxes; ++i) {
		vScores.insert(std::pair<float, int>(boundingBox_[i].score, i));
	}
	while (vScores.size() > 0) {
		unsigned int last = (unsigned int) vScores.rbegin()->second;
		vPick[nPick] = last;
		nPick += 1;
		for (std::multimap<float, int>::iterator it = vScores.begin(); it != vScores.end();) {
			unsigned int it_idx = (unsigned int) it->second;
...
void
Detect::refine(std::vector<Bbox> &vecBbox, const int &height, const int &width, bool square) {
	if (vecBbox.empty()) {
	...
	for (std::vector<Bbox>::iterator it = vecBbox.begin(); it != vecBbox.end(); it++) {
		...
		if (square) {
			...
			h = y2 - y1 + 1;
			maxSide = (h > w) ? h : w;
			x1 = (float) (x1 + w * 0.5 - maxSide * 0.5);
			y1 = (float) (y1 + h * 0.5 - maxSide * 0.5);
